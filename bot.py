...
def extract_and_apply_profile_update(reply: str, db=None, user_id=None) -> tuple[str, bool]:
...
def maybe_update_profile(message: str, db=None, user_id=None) -> bool:
    ...
    _, updated = extract_and_apply_profile_update(result, db=db, user_id=user_id)
    return updated
...
async def handle_main(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🏋️ Начать тренировку":
        ...
        await show_profile(update, db=db, user_id=update.effective_user.id)
...
async def handle_workout(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not current:
        ...
        clean_reply, _ = extract_and_apply_profile_update(reply, db=db, user_id=user_id)
    ...
    if set_match:
        ...
        if comment:
            maybe_update_profile(comment, db=db, user_id=user_id)
    ...
    await update.message.chat.send_action("typing")
    maybe_update_profile(text, db=db, user_id=user_id)
    profile = load_profile(db, user_id)
    ...
    clean_reply, updated = extract_and_apply_profile_update(reply, db=db, user_id=user_id)
    workout["chat_history"] += [
        {"role": "user", "content": text},
        {"role": "assistant", "content": clean_reply}
    ]
...
async def show_profile(update: Update, db=None, user_id=None):
    profile = load_profile(db, user_id) if db else DEFAULT_PROFILE
    if len(profile) > 3500:
